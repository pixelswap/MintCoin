...
contract WorldWarPool is Ownable {
    ...
    function earned(uint256 _pid, address account) public view returns (uint256) {
        ...
        //距离上次提取挖矿收益期间单个token的收益
        //当前矿池单个token的收益 减去 该用户上次提取时单个token的挖矿收益
    ...
    function rewardPerToken(uint256 _pid) public view returns (uint256) {
        ...
        return
        //rewardPerTokenStored初始值为0，阶段性递加
        pool.rewardPerTokenStored.add(
            //距离池子上次更新的时间间隔
            lastTimeRewardApplicable(_pid).sub(pool.lastUpdateTime)
            //单位时间奖励，初始值：_initReward.div(_duration)
            .mul(pool.rewardRate)
